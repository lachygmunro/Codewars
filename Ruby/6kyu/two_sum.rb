...
# these items should then be returned in a tuple / list (depending on your language) like so: (index1, index2).
...
def two_sum(numbers, target)
  numbers.each_with_index do |a, idx|
    numbers.each_with_index do |b, ind|
      total = a + b
      return [idx, ind] if total == target && idx != ind
      end
    end
end
